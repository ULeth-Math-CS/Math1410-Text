...
\definition{def:vectorcr}{Column and Row Vectors}{
...
Let \vu\ be an $1\times n$ row vector with entries $u_1$, $u_2$, $\cdots$, $u_n$ and let \vv\ be an $n\times 1$ column vector with entries $v_1$, $v_2$, $\cdots$, $v_n$. The \textit{product of \vu\ and \vv}, denoted $\vu\cdot\vv$ or $\vu\vv$, is 
\[
\sum_{i=1}^n  u_iv_i = u_1v_1 + u_2v_2 + \cdots + u_nv_n.
\]}
