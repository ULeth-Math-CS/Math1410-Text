...
We have seen this sort of creature before! From Definition \ref{def:matrix}, we know that the object we have just created is called a {\em matrix}. We will often want to distinguish matrices with just the coefficients of a system of linear equations (without the constants from the right-hand sides) and matrices that do contain the column of constants, as above. Therefore, when we include the constants, we often refer to the resulting matrix as an \textit{augmented matrix}.\index{augmented matrix}\index{matrix!augmented}
