...
\]}
{The solution is rather straightforward, even though we did a lot of work before to find the answer. Form the augmented matrix $\bmx{cc} \tta&\vb\emx$ and interpret its \rref.
\[
\bmx{cccc}1&2&3&5\\-1&2&1&-1\\1&1&0&2\emx\quad\overrightarrow{\text{rref}}\quad \bmx{cccc}1&0&0&2\\0&1&0&0\\0&0&1&1\emx
\]
...
\example{ex_vect_sol_102}{Another matrix equation}{Solve the linear system $\tta\vx=\zero$ and write the solution in vector form, where 
\[
\tta = \bmx{cc}2 & -3 \\ -2 & 3\emx.
\]
}
{Again, to solve this problem, we form the proper augmented matrix and we put it into \rref, which we do below. 
\[
...
\example{ex_vect_sol_103}{A matrix equation with non-zero right-hand side}{Solve the linear system $\ttaxb$, where 
\[
...
\keyidea{idea:unique_solutions}{Solutions of Consistent Systems}{\index{homogeneous}\index{consistent}\index{system of linear equations!consistent}\index{solution!unique}\index{solution!infinite}
...
\example{ex_vector_solution_4}{Solving a homogeneous and non-homogeneous system}{Let 
\[
\tta = \bmx{cccc}1&-1&1&3\\4&2&4&6\emx \quad \text{and} \quad \vb = \bmx{c}1\\10\emx.
\]
Solve the linear systems $\tta\vx = \zero$ and $\tta\vx = \vb$ for \vx, and write the solutions in vector form.}
{We'll tackle $\tta\vx = \zero$ first. We form the associated augmented matrix, put it into \rref, and interpret the result. 
\[
...
So $\vect{x_p}$ is a particular solution; $\tta\vxp = \vb$. (Multiply it out to verify that this is true.) The other vectors, \vu, \vv\ and \vect{w}, that are multiplied by our free variables $x_3$, $x_4$ and $x_5$, are each solutions to the homogeneous equations, $\tta\vx=\zero$. Any linear combination of these three vectors, i.e., any vector found by choosing values for $x_3$, $x_4$ and $x_5$ in $x_3\vu+x_4\vv+x_5\vect{w}$ is a solution to $\tta\vx=\zero$.}
