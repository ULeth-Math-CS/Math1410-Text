...
\example{ex_draw_vectors_1}{Sketching vectors}{Let 
\[
\vx = \bmx{c}1\\-1\emx \quad \vy = \bmx{c} 2\\3\emx \quad \text{ and } \quad \vz = \bmx{c} -3\\2\emx.
\]
...
\mnote{.5}{Vectors are just special types of matrices. The zero vector, \zero, is a special type of zero matrix, \tto. It helps to distinguish the two by using different notation.} 
...
\example{ex_add_vectors_1}{Sketching vectors and their sum}{Let 
\[
\vx = \bmx{c}1\\1\emx \quad \text{ and } \quad \vy = \bmx{c}3\\1\emx.
\]
Sketch \vx, \vy\  and $\vx+\vy$.}
...
\ifthenelse{\boolean{colour}}{\fill[color=blue!10] (0,0)--(1,1)--(4,2)--(3,1)--(0,0);}
...
\keyidea{idea:Parallelogram}{Parallelogram Law}{
...
\example{ex_draw_add}{Addition of vectors}{Consider the vectors \vx\ and \vy\ as drawn in Figure \ref{fig:draw_add_1}. Sketch the vector $\vx+\vy$.}
{
\begin{myfigure}%[h!]
\btz[>=latex,scale=.75]
\draw (-3,0)--(2.5,0);
...
\example{ex_scalar_vect_1}{Visualizing scalar multiplication}{Let 
\[
\vx = \bmx{c}1\\1\emx \quad \text{ and } \quad \vy = \bmx{c}-2\\1\emx.
\]
Sketch \vx, \vy, 3\vx\ and $-1\vy$.}
{We begin by computing 3\vx\ and $-\vy$: 
\[
...
\example{ex_draw_scale_1}{Sketching scalar multiples}{Let vectors \vx\ and \vy\ be as in Figure \ref{fig:draw_scale_1}. Draw 3\vx, $-2\vx$, and $\frac12\vy$.
...
\example{ex_subtract}{Sketching the difference of two vectors}{Let vectors \vx\ and \vy\ be as in Figure \ref{fig:subtract_1}. Draw $\vx-\vy$.
...
\keyidea{idea:vector_subtract}{Vector Subtraction}{
...
\example{ex_subtract_quick}{Sketching a vector difference}{Let \vx\ and \vy\ be as in Figure \ref{fig:subtract_quick_2} (a). Draw $\vx-\vy$.
%
%\begin{myfigure}%[h!]
%\btz[>=latex,scale=.6]
%\draw (-.5,0)--(1.5,0);
...
\definition{def:vector_length}{Vector Length}{\index{vector!length}

Let 
\[
\vx = \bmx{c}x_1\\x_2\emx.
\]
The \textit{length} of \vx, denoted $\len{\vx}$, is 
\[
\len{\vx} = \sqrt{x_1^2+x_2^2}.
\]}

\medskip

\example{ex_length}{Calculating the length of vectors}{Find the length of each of the vectors given below.
\[
...
\example{ex_scalar_vect_2}{Measuring the effect of scalar multiplication on vector length}{Let $\vx = \bmx{c} 2\\-1\emx$. Compute $\len{\vx}$, $\len{3\vx}$, $\len{-2\vx}$, and $\len{c\vx}$, where $c$ is a scalar.}
...
\theorem{thm:vect_length}{Vector Length and Scalar Multiplication}{

Let \vx\ be a vector and let $c$ be a scalar. Then the length of $c\vx$ is 
\[
\len{c\vx} = |c|\cdot \len{\vx}.
\]}

\noindent \large \textsf{\textbf{Matrix $-$ Vector Multiplication}} \normalsize\\

The last arithmetic operation to consider visualizing is matrix multiplication. Specifically, we want to visualize the result of multiplying a vector by a matrix. In order to multiply a 2D vector by a matrix and get a 2D vector back, our matrix must be a square, $2\times 2$ matrix.
